...
//write this function as a callback inline function
//change sayHelloyButton background color to black using .style
// change sayHelloyButton text color to white using .style
...
//another inline callback function, this time using "mouseout" as the event
...
// CODE HERE
// format: object.addEventListener('event', functionName)
...
const repeatMyParam = () => {
  axios.get("http://localhost:3000/repeat/I-love-pink-color").then((res) => {
    let repeatElement = document.getElementById("repeat-text");
    repeatElement.textContent = res.data;
    repeatElement.style.display = "block";
    repeatElement.style.backgroundColor = "pink";
    repeatElement.style.color = "purple";
  });
};
...
const askFavColor = () => {
  ...
  const queryParam = `color=${favoriteColor}`;
  axios
    .get(`http://localhost:3000/query-test?${queryParam}`)
    .then((response) => {
      console.log(response.data);
      let queryColor = document.getElementById("query-text");
      queryColor.textContent = response.data;
      queryColor.style.display = "block";
      queryColor.style.backgroundColor = favoriteColor; // Use favoriteColor variable
      queryColor.style.color = "black";
    })
    .catch((error) => console.error(error));
};

document.getElementById("query-button").addEventListener("click", askFavColor);
